...
final class OgImageGenerator extends Page
{
    ...
    /** @var array{source: ?string, entry: int|string|null} */
    public array $data = [
        'source' => null,
        'entry' => null,
    ];
    ...
    /** @return null|array{template: string, data: object} */
    public function preview(): ?array
    {
        $source = $this->selectedSource();
        $entry = $this->data['entry'];

        if ($source === null || (! is_int($entry) && ! is_string($entry))) {
            return null;
        }
        ...
        return [
            ...
            'data' => $source->resolveData($record),
        ];
    }

    public function regenerate(OgImageManager $manager): void
    {
        $state = $this->generatorForm()->getState();
        $source = $state['source'] ?? null;
        $entry = $state['entry'] ?? null;

        if (! is_string($source) || (! is_int($entry) && ! is_string($entry))) {
